...
data = np.loadtxt('ugc914_pos_wav.dat')
...
ax1.errorbar(afstandc,vel+24.7,yerr=sigma_vel,xerr=sigma_afs,fmt='.')
...
Rd = 3.9*3.086*10**19 # disk scale length
c = 3.0 * 10 ** 5 #speed of light [km/s]
...
plt.figure()
plt.grid()
plt.errorbar(afstandc+1.0,vel,yerr=sigma_vel,xerr=sigma_afs,fmt='.', label='Målinger')
plt.ylabel('Rotationshastighed i km/s')
plt.xlabel('Afstand fra centrum i kpc')
plt.plot(x,vc_r,color='tab:red',label='Forventet hastighed uden mørkt stof')
